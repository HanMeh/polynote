...
object VirtualEnvFetcher {
  ...
  private def installDependencies(
    ...
    type Updater = (TaskInfo => TaskInfo) => Unit
    def registerInstallationTasks(
      taskManager: TaskManager.Service,
      currentTaskId: String,
      refs: Map[String, AtomicReference[Updater]]
    ) =
      ZIO.foreach_(dependencies)(dep => taskManager.register(dep, dep, "installing", Some(currentTaskId)){
        updater =>
          refs(dep).set(updater)
          ZIO.unit
      })
    ...
    def installDependency(
      ...
      for {
        _ <- ZIO(Option(refs(dep).get).foreach(_(_.progress(0.1))))
        _ <- currentTask.update(task => task.progress(task.progressFraction + depInitProgress, Some(s"Installing $dep")))
        _ <- pip("install", dep)
        _ <- ZIO(Option(refs(dep).get).foreach(_(_.progress(0.8))))
        _ <- currentTask.update(task => task.progress(task.progressFraction + depInstalledProgress))
        _ <- pip("download", dep, List("--dest", s"$venv/deps/"))
        _ <- ZIO(Option(refs(dep).get).foreach(_(_.completed)))
        _ <- currentTask.update(task => task.progress(task.progressFraction + depDownloadedProgress))
      } yield ()
    }
    ...
    for {
      ...
      currentTaskId <- CurrentTask.get.map(_.id)
      refs           = dependencies.map(_ -> new AtomicReference[(TaskInfo => TaskInfo) => Unit]()).toMap
      _             <- registerInstallationTasks(taskManager, currentTaskId, refs)
      _             <- ZIO.foreach_(dependencies)(dep => installDependency(dep, refs, currentTask))
    } yield ()
  }
